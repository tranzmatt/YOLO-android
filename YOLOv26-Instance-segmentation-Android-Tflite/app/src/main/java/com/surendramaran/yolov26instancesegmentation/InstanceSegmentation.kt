...
import com.surendramaran.yolov26instancesegmentation.ImageUtils.clone
import com.surendramaran.yolov26instancesegmentation.ImageUtils.scaleMask
import com.surendramaran.yolov26instancesegmentation.ImageUtils.toMask
import com.surendramaran.yolov26instancesegmentation.MetaData.extractNamesFromMetadata
...
class InstanceSegmentation(
    ...
    private fun bestBoxYolo26(array: FloatArray) : List<Output0>? {
        ...
        for (detection in 0 until numChannel) {
            ...
            val x1 = normalizeX(array[rowOffset]).coerceIn(0F, 1F)
            val y1 = normalizeY(array[rowOffset + 1]).coerceIn(0F, 1F)
            val x2 = normalizeX(array[rowOffset + 2]).coerceIn(0F, 1F)
            val y2 = normalizeY(array[rowOffset + 3]).coerceIn(0F, 1F)
            val w = x2 - x1
            val h = y2 - y1
    ...
    private fun normalizeX(value: Float): Float {
        return if (value > 1F) value / tensorWidth else value
    }

    private fun normalizeY(value: Float): Float {
        return if (value > 1F) value / tensorHeight else value
    }
